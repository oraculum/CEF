    ...
    internal class Banco_HSBC : AbstractBanco, IBanco
    {
        ...
        public override void ValidaBoleto(Boleto boleto)
        {
            try
            {
                ...
                if (Convert.ToInt32(boleto.NossoNumero).ToString().Length > 7)
                    throw new NotImplementedException("A quantidade de dígitos do nosso número para a carteira " + boleto.Carteira + ", são 7 números.");
                else if (Convert.ToInt32(boleto.NossoNumero).ToString().Length < 7)
                    boleto.NossoNumero = Utils.FormatCode(boleto.NossoNumero, 7);
            ...
            try
            {
                ...
                switch (boleto.Carteira.ToUpper())
                {
                    ...
                    case "CNR":
                        ...
                        EEEEEEEE = numeroDocumento.Substring(0, 8);
                        ...
                        FFFFF = numeroDocumento.Substring(8, 5);
        ...
        public override void FormataNossoNumero(Boleto boleto)
        {
            try
            {
                switch (boleto.Carteira.ToUpper())
                {
                    ...
                    case "CNR": 
                        boleto.NossoNumero = string.Format("{0}{1}4{2}", boleto.NossoNumero, Mod11Base9(boleto.NossoNumero).ToString(), Mod11Base9((int.Parse(boleto.NossoNumero + Mod11Base9(boleto.NossoNumero).ToString() + "4") + int.Parse(boleto.Cedente.Codigo.ToString()) + int.Parse(boleto.DataVencimento.ToString("ddMMyy"))).ToString())); 
                        break;
                    default:
